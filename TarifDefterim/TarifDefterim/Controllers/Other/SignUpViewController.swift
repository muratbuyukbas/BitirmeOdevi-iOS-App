...
class SignUpViewController: UIViewController, UITextFieldDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    private let signUpButton: UIButton = {
        ...
        button.backgroundColor = UIColor(red: 216/255, green: 67/255, blue: 57/255, alpha: 1)
    ...
    private let termsButton: UIButton = {
        let button = UIButton()
        button.setTitleColor(UIColor(red: 55/255, green: 50/255, blue: 52/255, alpha: 1), for: .normal)
        button.setTitle("Kullanım Şartları", for: .normal)
        return button
    }()
    ...
    private let privacyButton: UIButton = {
        let button = UIButton()
        button.setTitleColor(UIColor(red: 55/255, green: 50/255, blue: 52/255, alpha: 1), for: .normal)
        button.setTitle("Gizlilik Politikası", for: .normal)
        return button
    }()
    ...
    override func viewDidLoad() {
        ...
        addButtonActions()
        addImageGesture()
        self.navigationController!.navigationBar.tintColor = UIColor(red: 216/255, green: 67/255, blue: 57/255, alpha: 1)
        self.navigationController!.navigationBar.titleTextAttributes = [.foregroundColor: UIColor(red: 55/255, green: 50/255, blue: 52/255, alpha: 1)]
    ...
    @objc func didTapImage() {
        let sheet = UIAlertController(
            ...
            message: "Profil fotoğrafı ekleyerek arkadaşlarının seni daha rahat bulmasını sağlayabilirsin.",
            preferredStyle: .actionSheet
        )

        sheet.addAction(UIAlertAction(title: "Kapat", style: .cancel, handler: nil))
        sheet.addAction(UIAlertAction(title: "Fotoğraf Çek", style: .default, handler: { [weak self] _ in
        ...
        sheet.addAction(UIAlertAction(title: "Galeriden Seç", style: .default, handler: { [weak self] _ in
            DispatchQueue.main.async {
                let picker = UIImagePickerController()
                picker.allowsEditing = true
    ...
    private func presentError() {
        let alert = UIAlertController(title: "Hata", message: "Lütfen gerekli alanları doldurduğundan emin ol ayrıca şifre uzunluğu 6'dan büyük olmalı", preferredStyle: .alert)
        alert.addAction(UIAlertAction(title: "Kapat", style: .cancel, handler: nil))
        present(alert, animated: true)
    }

    @objc func didTapTerms() {
        guard let url = URL(string: "https://www.muraat.com") else {
            return
        }
